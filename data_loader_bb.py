...
class CustomDataSet(torch.utils.data.TensorDataset):
    ...
    def __getitem__(self, idx):
        ...
        path = os.path.join(self.params.main_task_img_dir, img_id)
...
class DataLoader:
    def __init__(self, params):
        ...
        self.train_data_loader = torch.utils.data.DataLoader(dataset_train,
                                                             ...
                                                             shuffle=True, **kwargs)
        dataset_val = CustomDataSet(params, self.x, self.x_flair, self.y,  self.img_id, self.datasplit[1], self.datasplit[2],self.ifpairs,self.indexs_list)
        self.val_data_loader = torch.utils.data.DataLoader(dataset_val,
                                                           batch_size=1,
                                                           collate_fn=dataset_val.collate,
                                                           shuffle=False, **kwargs)
        dataset_test = CustomDataSet(params, self.x,  self.x_flair, self.y, self.img_id, self.datasplit[2], self.datasplit[3],self.ifpairs,self.indexs_list)
    ...
    def load_data(self):
        ...
        datasplit, sentences, sent_maxlen, word_maxlen, num_sentence,  img_id ,ifpairs= self.load_sentence(
            'IMGID', self.params.main_task_split_file, 'train', 'dev', 'test')
    ...
    def load_sentence(self, IMAGEID, tweet_data_dir, train_name, dev_name, test_name):
        ...
        for fname in (train_name,dev_name,test_name):
            ...
            import csv
            with open(os.path.join(tweet_data_dir, fname), "r", encoding="utf-8") as file: #12-13列
                reader = csv.reader(file,delimiter="	")
                birth_header = next(reader)  
                for line in reader:
                    num=line[4]
                    img_id.append(num)

                    sentence=line[5].split()
                    ...
                    elif self.params.class_name=="img":
                        ifpairs.append(int(line[2]))
                    else:
